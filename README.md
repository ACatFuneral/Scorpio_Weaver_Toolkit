...
| ախ **智能诊断 (v5.3+)** | 能精准识别因AI模型输出超限导致的失败，并提供清晰的错误报告，不再被模糊的网络错误困扰。 |
...
> 这是整个流程的核心，**大部分操作只需要做一次**！以后无论脚本怎么更新，你的配置都会被保留。    

1.  **下载工具包**
    *   前往本项目的 [**Releases 页面**](https://github.com/ACatFuneral/Scorpio_Weaver_Toolkit/releases/latest)。
    *   下载最新版本的 `.zip` 压缩包（例如 `Scorpio.Weaver.Toolkit.v6.0.zip`）。
    *   解压后，你会得到一个包含 `scripts` 文件夹和 `README.txt` 的工具包。

*   **重要提示**: `config.json` 文件**必须**和 `Scorpio_Weaver.py`、`Glossary_Forge.py` 脚本放在同一个文件夹里！

2.  **首次配置 (向导模式)**
    *   打开命令行/终端，进入你解压后的工具包目录。
    *   执行以下命令，启动配置向导：
      ```bash
...
3.  **日常修改配置 (可选)**
    *   如果你需要修改配置（比如换个游戏处理），直接用记事本或 VS Code 打开 `scripts/config.json` 文件修改即可。**无需再动 `.py` 文件！**
    *   **【警告！】**  
    -   你**永远**只需要修改 `config.json` 这一个文件来调整配置。  
    -   脚本文件（`.py`）内部的 `default_config` 仅用于首次生成 `config.json`，**修改它不会产生任何效果！**  
    *   **【重要】** `config.json` 里的路径分隔符，请使用**双反斜杠 `\\`**，例如 `C:\\Games\\MyGame\\game`。
...
> 配置完成后，后续所有操作都从这里开始。这是最核心的工具，用于批量给代码添加翻译标记。
