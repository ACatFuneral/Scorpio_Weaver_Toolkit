...
-   ⚔️ **游侠版 (`Scorpio_Weaver_Rogue.py`)**: 正则驱动，极速免费，需要搭配手动正则决战，适合所有追求效率和零成本的用户。
...
| 特性 | 描述 |
| :--- | :--- |
| ✌️ **双子核心 (v7.0+)** | **双剑合璧！** 同时提供 **AI旗舰版** 和 **正则游侠版** 两大核心，满足不同用户的全部需求！|
...
### v7.0 - 双子核心版 (YYYY-MM-DD)
*   **【创世更新】引入“正则游侠版” (`Scorpio_Weaver_Rogue.py`)**！
    *   **零成本，零门槛**: 完全摆脱对API的依赖，下载即用，无需任何Key！
    *   **快如闪电**: 基于本地正则处理，速度是AI版的数百倍。
    *   **绝对精准**: 只处理 `Character()` 等绝对安全的文本模式，宁可漏过，绝不錯杀。
    *   **人机协同**: 完美配合手动正则模式，形成“游侠初筛，英雄决战”的终极工作流。
*   **【架构统一】** 重构项目，为“AI旗舰版”和“正则游侠版”分别配备了独立的配置文件 (`config.json` 和 `config_rogue.json`) 和配置向导。
*   **【文档飞升】** `README.md` 全面重构，引入版本选择和双线流程，用户体验MAX！
...
### 作战路线 A：驾驶AI旗舰 (`Scorpio_Weaver.py`)  
*   在你的“军火库”终端里，执行：
    ```bash
...
### 作战路线 B：召唤正则游侠 (`Scorpio_Weaver_Rogue.py`)
...
4.  **手动补完**:
    *   “游侠”只会处理最安全的部分。对于剩余的文本，请务必使用 **【附录D】** 中的**手动正则决战**方法进行最终打击！  
