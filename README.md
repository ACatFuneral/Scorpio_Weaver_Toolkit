...
3.  **解压入库**：将下载的 `.zip` 包解压，把里面的 `scripts` 文件夹完整地复制到你的“军火库”中。
*   **重要提示**: `config.json` 文件**必须**和 `Scorpio_Weaver.py`、`Glossary_Forge.py` 脚本放在同一个文件夹里！最终结构应如下：
    ```
    D:\MyToolkit\
    └── scripts\
        ├── Scorpio_Weaver.py
 ...
 **注意：** 这一步不光要求速度，道德审核也很严重。有报错是正常的，毕竟API有波动。如果报错超多，或遇到道德审核问题，别着急，请直接跳到下面的 **【最终手段：手动正则决战】**。
...
*   **咒语五：查找通用函数调用中的字符串参数**
    ...
    \w+\s*\(\s*[^)]*\b"([^"/]+?)"
    ```
    > **注**：咒语五威力巨大，但可能误伤友军，请谨慎使用并仔细甄别结果。
...
<details>
<summary><strong>🪄 第五步：替换字体 (点击展开)</strong></summary>
