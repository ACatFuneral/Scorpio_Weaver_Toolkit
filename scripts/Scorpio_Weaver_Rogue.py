...
# ==============================================================================
#  Scorpio Weaver Toolkit - Rogue Edition v2.2 (Final Hotfix)
#  作者：猫之送葬者 & Gemini
...
def load_config():
    ...
    if not os.path.exists(config_path):
        ...
        with open(config_path, 'w', encoding='utf-8') as f: json.dump(default_config, f, indent=2, ensure_ascii=False)
...
def config_wizard():
    ...
    while True:
        ...
        if os.path.isdir(game_dir): config['GAME_DIRECTORY'] = game_dir; break
        else: print("❌ 目录不存在，请重新输入")
    ...
    while True:
        try:
            ...
            if concurrent > 0: config['CONCURRENT_LIMIT'] = concurrent; break
...
PAUSE_EVENT.set()
STOP_EVENT = threading.Event() # 【史诗级修复】把被我删掉的“紧急停止按钮”给你装回来了！

# --- 核心模块 ---
def install_and_import_tqdm():
    try: import tqdm; return tqdm
    except ImportError:
        ...
        except Exception as e: logger.error(f"✘ 'tqdm' 自动安装失败: {e}\n请手动运行 'pip install tqdm'。"); return None
...
def main():
    ...
    # 将 CONFIG 设为全局变量，以便在函数内部访问
    global CONFIG
    CONFIG = load_config()
    validate_config(CONFIG)

    logger.info("\n" + "⚔️ "*30 + f"\n  Scorpio Weaver - Rogue Edition v2.2  \n" + "⚔️ "*30)
